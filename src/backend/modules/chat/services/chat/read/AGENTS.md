...
- Path: `packages/apps/chat/src/services/chat/read`
...
- Functions (all):
  - `fetchChatMessages` (L7-L7, function)
- Exports:
  - `fetchChatMessages` (L7)
- Key snippets and use-case mapping:
  - `L7-L7`: Implements `fetchChatMessages` for this module use case.
### `query-chat-engine.ts`
- Purpose: Defines module behavior owned by this usage folder.
- Owning use cases: Runtime and application flows that import this file through package boundaries.
- Input shape: Typed arguments and imported contracts declared in this file signatures.
- Output shape: Typed return values, thrown errors, and exported contracts declared in this file.
...
- Logic gates summary:
  ...
  - MCP or GraphQL proxy boundaries must avoid duplicated domain validation.
- Functions (all):
  - `chatAttachmentInputs` (L12-L12, arrow)
- Exports:
  - `chatAttachmentInputs` (L12)
- Key snippets and use-case mapping:
  - `L12-L12`: Implements `chatAttachmentInputs` for this module use case.
### `query-chat.ts`
- Purpose: Defines module behavior owned by this usage folder.
- Owning use cases: Runtime and application flows that import this file through package boundaries.
- Input shape: Typed arguments and imported contracts declared in this file signatures.
...
- Logic gates summary:
  ...
  - MCP or GraphQL proxy boundaries must avoid duplicated domain validation.
- Functions (all):
  - `resolveSessionScope` (L58-L58, function)
  - `queryChat` (L99-L99, function)
- Exports:
  - `resolveSessionScope` (L58)
  - `queryChat` (L99)
- Key snippets and use-case mapping:
  - `L58-L58`: Implements `resolveSessionScope` for this module use case.
  - `L99-L99`: Implements `queryChat` for this module use case.

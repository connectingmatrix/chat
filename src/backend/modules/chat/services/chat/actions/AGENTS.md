...
- Path: `packages/apps/chat/src/services/chat/actions`
...
- Logic gates summary:
  ...
  - MCP action dispatch for this folder routes through `@giga/mcp` chat-action handlers.
- Functions (all):
  - `mcpRequest` (L26-L26, arrow)
  - `mcpDispatch` (L28-L28, function)
  - `executeGigaAction` (L53-L53, function)
- Exports:
  - `GIGA_ACTION_CATALOG` (L7)
  - `getReadOnlyGigaActionCatalog` (L7)
  - `isGigaAction` (L7)
  - `isMutatingAction` (L7)
  - `executeGigaAction` (L53)
- Key snippets and use-case mapping:
  - `L26-L26`: Implements `mcpRequest` for this module use case.
  - `L28-L28`: Implements `mcpDispatch` for this module use case.
  - `L53-L53`: Implements `executeGigaAction` for this module use case.
